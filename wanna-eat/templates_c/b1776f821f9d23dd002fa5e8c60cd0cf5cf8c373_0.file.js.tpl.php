<?php
/* Smarty version 3.1.34-dev-7, created on 2020-07-03 06:26:03
  from 'D:\xampp\htdocs\wanna-eat\wanna-eat\templates\js.tpl' */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '3.1.34-dev-7',
  'unifunc' => 'content_5efe5efbf18909_23020780',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    'b1776f821f9d23dd002fa5e8c60cd0cf5cf8c373' => 
    array (
      0 => 'D:\\xampp\\htdocs\\wanna-eat\\wanna-eat\\templates\\js.tpl',
      1 => 1593230006,
      2 => 'file',
    ),
  ),
  'includes' => 
  array (
  ),
),false)) {
function content_5efe5efbf18909_23020780 (Smarty_Internal_Template $_smarty_tpl) {
echo '<script'; ?>
 src="./language/js/jquery.js"><?php echo '</script'; ?>
>
<?php echo '<script'; ?>
 src="https://cdnjs.cloudflare.com/ajax/libs/vue/2.6.11/vue.js"><?php echo '</script'; ?>
>
<?php echo '<script'; ?>
 src="https://cdnjs.cloudflare.com/ajax/libs/axios/0.19.2/axios.min.js"><?php echo '</script'; ?>
>
<?php echo '<script'; ?>
 src="./language/js/bootstrap.js"><?php echo '</script'; ?>
>
<?php echo '<script'; ?>
 src="./language/js/custom.js"><?php echo '</script'; ?>
>
</body>
</html><?php }
}
